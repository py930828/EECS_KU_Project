...
int both_chop_available (philosopher *p)
{
...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    pthread_mutex_lock( &waiter ); // the waiter is checking if anyone wants to eat
    while( !( both_chop_available(me) ) ) // checking if this philosopher is able to eat
    {
      pthread_cond_wait( &(me->can_eat), &waiter ); // wait for the chopstick to be available
    }
    *left_chop_available( me ) = 0;
    *right_chop_available( me ) = 0; // waiter gives the chopsticks both chopstick taken
    pthread_mutex_unlock( &waiter ); //waiter goes to serve someone else
    ...
    *right_chop_available( me ) = 1; // philosopher puts down the chopsticks
