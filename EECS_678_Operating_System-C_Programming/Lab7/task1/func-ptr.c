...
{
	int this_priority = ( (Process *)this)->priority;
	int that_priority = ( (Process *)that)->priority;
	int arrival_time_diff = ( (Process *)this)->arrival_time - ( (Process *)that)->arrival_time;
	int result = that_priority - this_priority;
	if( !result )
	{
		return arrival_time_diff;
	}
	else
	{
		return result;
