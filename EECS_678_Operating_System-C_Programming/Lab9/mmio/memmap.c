...
int main (int argc, char *argv[])
{
   ...
   */
   ...
   {
     err_sys("Failed on lseek.");
   }
  /*
   * 3. write a dummy byte at the last location
   */
   if( write(fdout, "s", 1) < 0 )
   {
     err_sys("Failed on write.");
   }
   printf("goes here???\n");
   ...
   */
   ...
   if( src < 0 )
   {
     err_sys("Failed on mmap.");
   }
  /*
   * 5. mmap the output file
   */
   dst = mmap( NULL, fileStat.st_size,  (PROT_READ | PROT_WRITE), MAP_SHARED, fdout, 0 );
   if( dst < 0 )
   {
     err_sys("Failed on mmap.");
   }
